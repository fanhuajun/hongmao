...
<script type="text/javascript" src="${base}/static/admin/js/wxmenu.js"></script>
	...
	<div class="zxbody" style="position:relative;">
		...
		<div class="extend_tab">
			<span id="tab1" onclick="window.location.href='agent!list.action?level=one'" <#if level=="one">class="on selected"</#if>>一级代理</span>
			<span id="tab2" onclick="window.location.href='agent!list.action?level=two'" <#if level=="two">class="on selected"</#if>>二级代理</span>
			<span id="tab3" onclick="window.location.href='agent!list.action?level=three'" <#if level=="three">class="on selected"</#if> >三级代理</span>
		</div>
...
function getContentList(pn,stat){
	var url = "json/agent!list.action";
...
function showPage(pn,stat){
	...
	var url = "json/agent!totalNum.action";
